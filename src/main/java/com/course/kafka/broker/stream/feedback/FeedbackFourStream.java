...
public class FeedbackFourStream {
    ...
    public KStream<String, FeedbackMessage> kstreamFeedback(StreamsBuilder builder){
        ...
        feedbackStream[0].to("t.commodity.feedback-four-good");
        feedbackStream[0].groupByKey().count().toStream().to("t.commodity.feedback-four-good-count");

        feedbackStream[1].to("t.commodity.feedback-four-bad");
        feedbackStream[1].groupByKey().count().toStream().to("t.commodity.feedback-four-bad-count");
