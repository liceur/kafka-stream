...
public class FeedbackSixStream {
    ...
    public KStream<String, FeedbackMessage> kstreamFeedback(StreamsBuilder builder){
        ...
        feedbackStream[0].through("t.commodity.feedback-six-good")
                         .groupByKey()
                         .count()
                         .toStream().to("t.commodity.feedback-six-good-count");

        feedbackStream[1].through("t.commodity.feedback-six-bad")
                         .groupByKey()
                         .count()
                         .toStream().to("t.commodity.feedback-six-bad-count");

        feedbackStream[0].groupBy((k,v)-> v).count().toStream().to("t.commodity.feedback-six-good-count-word");
        feedbackStream[1].groupBy((k,v)-> v).count().toStream().to("t.commodity.feedback-six-bad-count-word");
