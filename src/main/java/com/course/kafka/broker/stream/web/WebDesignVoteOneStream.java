...
public class WebDesignVoteOneStream {
    ...
    public KStream<String, WebDesingVoteMessage> kstreamWebDesingVote(StreamsBuilder builder){
        ...
        builder.stream("t.commodity.web.vote-color",
                ...
                .mapValues(v -> v.getColor()).to("t.commodity.web.vote-one-stream-color");
        ...
        builder.stream("t.commodity.web.vote-layout",
                ...
                .mapValues(v -> v.getLayout()).to("t.commodity.web.vote-one-stream-layout");
        var layaoutTable = builder.table("t.commodity.web.vote-one-username-layout",
                Consumed.with(stringSerde, stringSerde));

        // join
        var joinTable = colorTable.join(layaoutTable, this::voteJoiner, Materialized.with(stringSerde, desingSerde));
        ...
        joinTable.groupBy((username, voteDesign) -> KeyValue.pair(voteDesign.getLayout(), voteDesign.getLayout()))
                .count().toStream().print(Printed.<String, Long>toSysOut().withLabel("Vote one - layaout"));
